...
- **降下方向は垂直モードと斜めモードを交互に切り替える**（`RAIN_MODE_DURATION` 秒ごと、初期値10秒）
  ...
  - 見た目（雨粒のストリークの向き）も実際の風速に応じて傾ける（風速のなめらかな変化に追従する）。
    **リプレイ中も同様に傾ける**：`Recorder`が各フレームの風速（`windX`/`windZ`）を記録し、
    `Replayer.frame` 経由で再現する（プレイ中に斜めモードだった場面は、リプレイでも斜めに見える）
- 同時に存在する雨粒数は時間経過で増える：開始直後は `RAIN_COUNT_MIN = 60`（初期値150は実機テストで
  「多すぎ・避けられない」との評価だったため減量）、そこから `GAME_DURATION` いっぱいをかけて
  `RAIN_COUNT_MAX = 130` まで増加する（時間経過による難易度上昇。4.2参照）。線形に増やすと
  変化がほぼ体感できなかった（実機フィードバック）ため、増加量は2乗カーブで終盤に集中させている
- 初期の難易度方針：**「よけられる」ことを最優先**。密度・速度は実機テストしながら調整する前提
- PLAYING開始時、`RAIN_COUNT_MIN` 個の雨粒を一気に出現させない。`RAIN_RAMP_UP_DURATION` 秒
  （初期値2.5秒）かけて少量ずつ高頻度で投入し、開始時の下限に達する
...
- `RAIN_SPEED_SLOW`、`RAIN_COUNT_MIN`／`RAIN_COUNT_MAX`、`GAME_DURATION` は全て `constants.js` で
  一元管理し、後から数値調整のみで難易度を変えられるようにする
- **時間経過による難易度上昇を実装済み**：同時に存在する雨粒数が `RAIN_RAMP_UP_DURATION` 秒後の
  `RAIN_COUNT_MIN` から、`GAME_DURATION` いっぱいをかけて `RAIN_COUNT_MAX` まで増える
  （`RainPhysics._targetActiveCount()`）。増加量は2乗カーブで、序盤はほぼ変わらず終盤に
  一気に密度が上がるようにしている（線形だと変化が体感できないという実機フィードバックにより変更）
...
- その時点のPICO Motion Trackerデータ（`MotionTrackerBridge.getLatest()`、未接続ならnull）
  … リプレイ中の脚の動き再現用。詳細は6.2参照
...
- アバターの見た目：人型glTFモデル（`PlayerAvatar.js`）。頭は記録データの頭姿勢をそのまま反映、
...
- **脚（PICO Motion Tracker連携、拡張機能）**：足首2個のトラッカーがあれば、脚もTwo-Bone IKで
  動かす。「リプレイの全身の動きが分かりづらい」というフィードバックを受けて追加した。
  - トラッカー本体は同一端末上で動く**別プロジェクトのネイティブ補助アプリ**（Unity + PICO
    Integration SDK）が読み、腰(Hips)からの相対オフセット（体の正面基準、メートル）をローカル
    WebSocket（`MOTION_TRACKER_WS_URL`、既定 `ws://127.0.0.1:8787`）でJSON配信する：
    ```json
    { "left":  { "ankle": {"x":0,"y":0,"z":0}, "knee": {"x":0,"y":0,"z":0} },
      "right": { "ankle": {"x":0,"y":0,"z":0}, "knee": {"x":0,"y":0,"z":0} } }
    ```
    `x`=右方向 / `y`=上方向 / `z`=後方向（`-z`が前方。WebXRの頭・コントローラー姿勢と同じ規約）。
    `knee` は省略可（無ければ膝の曲げ方向は「体の前方かつやや下」で近似する）。
  - **絶対座標ではなく相対値にしている理由**：ネイティブアプリ側のXRセッションとWebXR側の
    XRセッションは別物のため、絶対的な原点が一致する保証がない。相対値（腰から見た位置）
    だけを送ってもらえば、既存の頭トラッキングで決まっている胴体位置にそのまま重ねられる
    ため、原点合わせが不要になる。
  - `MotionTrackerBridge.js`（core）が受信・保持し、`Recorder`が毎フレーム記録、`Replayer`が
    補間して再生する。ネイティブアプリが未起動・未接続でもゲームは通常通り動く（脚はバインド
    ポーズのまま）。
  - **未検証の前提**：スタンドアロンAndroidヘッドセットは基本的に同時に1つのXRアプリしか
    前面表示できないため、「PICO Browserでゲームを起動したまま、ネイティブアプリ側がXR
    セッションを張らずにMotion Trackerの関節poseだけを読めるか」はUnity側の実装時に要検証。
    これが成立しない場合、この連携方式自体が別のアプローチに置き換わる可能性がある。

> **技術的根拠**：WebXRの `local-floor` 参照空間は同一セッション内で原点が固定されるため、ゲーム中に記録した座標をリプレイでそのまま使い回せる。セッションを切らないことが条件。
> **実装上の注意（実機で発覚した不具合）**：`local-floor` 参照空間の y=0 は「本当の床」を指すはずだが、実機（PICO Browser＋端末の床キャリブレーション）ではこれが信用できない場合がある（直立時の head.y が 0.64 など明らかにおかしい値になることを実機で確認済み。原点が本当の床ではなく、セッション開始時点の頭の位置に近いものになっていたと考えられる）。これにより `RAIN_GROUND_Y = 0` を「床」と信じてしまうと、雨が実際の床（や、しゃがんだ頭・手の高さ）まで届かず、しゃがむと判定範囲の外に出て無敵になる不具合が起きていた。
> **対策**：座標系の絶対的な意味（y=0が本当に床かどうか）には依存しないようにし、`RAIN_GROUND_Y` を 0 ではなく `-1.0` など余裕を持ってマイナス側まで下げることで、多少原点がズレていてもしゃがんだ頭・手を確実にカバーできるようにした（`renderer.xr.setReferenceSpaceType("local-floor")` の明示指定自体は残してあるが、これだけでは解決しなかった）。
...
- [x] 雨をランダムに降らせ、頭・手の当たり判定で避けられる
- [x] 3ライフ制、30秒生存でクリア／被弾3回でゲームオーバー
- [x] 被弾時：画面フラッシュ + SE + コントローラー振動
- [x] プレイ内容（頭・手・雨の位置）を記録
- [x] リプレイをARのまま人型アバターで再生（プレイヤーが物理的に歩き回って観察）
- [x] リプレイ中の HUD 進行バー
- [x] RESULT 画面（クリア/ゲームオーバー + スコア）→ リプレイ/終了を選択
...
export const RAIN_COUNT_MAX = 130;      // 個（GAME_DURATIONかけて増える終盤の上限。150は「多すぎ」評価だった値のため少し下）
...
export const RAIN_GROUND_Y = -1.0;      // m（この高さで再出現。0ではなくマイナス側に余裕を持たせた理由は6.3参照）
export const RAIN_RAMP_UP_DURATION = 2.5; // 秒（PLAYING開始時、この時間をかけてRAIN_COUNT_MINまで少しずつ投入）
