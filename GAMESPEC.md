...
- **脚（疑似足アニメーション）**：実際の足トラッキングデータは持たないため、頭の動き（しゃがみ量・
  ...
  - **しゃがみ**：腰の高さの直近の最大値を「直立していたときの高さ」として保持し（下がらない、
    上がる方向にのみ更新。最初は減衰させていたが、しゃがみを数秒キープすると基準ごと下がって
    膝が伸びきってしまう不具合が実機フィードバックで発覚し、非減衰に変更した）、実際の腰の高さ
    との差から膝の曲がり具合を決める。しゃがんでいる間ずっと膝が曲がって見える
  - **足踏み**：頭の水平移動速度が一定以上のとき、左右交互に足を上げ下げ・前後に踏み出す
    アニメーションを重ねる（速く動くほど大きく踏み出す）
  - **Footボーンの手動追従**：使用モデルの骨階層は `Foot.L/R` が `LowerLeg.L/R` の子ではなく
    別枝になっている（元のFBXリグのIK制御用ボーンがそのまま残っていると思われる）。そのため、
    すねを曲げても足先パーツが追従せず取り残される問題があった（実機フィードバックで発覚）。
    バインドポーズ時点の「LowerLegから見たFootの相対位置・相対回転」を保存しておき、毎フレーム
    LowerLegの現在の姿勢に合わせてFootのローカルtransformを直接計算する（擬似的な親子付け）
    ことで解消した
