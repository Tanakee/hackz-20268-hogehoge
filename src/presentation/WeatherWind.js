 ...
 * 向きと傾きに反映させる。プレイヤーには方位磁石も無いし、外の天気が難易度を
 * 左右しても誰も得しないが、on-theme（妄想の雨に現実の風を吹かせる）なのでやる。
 *
 * - データ元：Open-Meteo（APIキー不要・CORS開放・無料）
 * - 位置：?lat=&lon= のURL指定 → geolocation（ヘッドセットでは失敗しがち）→ FALLBACK
...
// 天気が取れなかったとき・位置が分からないときに使う座標（会場に合わせて書き換え可）。
const FALLBACK = { lat: 35.6812, lon: 139.7671, label: "東京" };
...
/**
 * 気象の「風向」= 風が吹いてくる方角。ゲーム内の水平ドリフトは
 * 風が吹いていく向きなので +180 する。
 * コンパス方位 → ワールド：北 = -Z、東 = +X（セッション開始時の向き基準）。
 * プレイヤーに方位磁石はないので厳密な北合わせは重要ではなく、内部整合だけ取る。
 * 返り値は RainPhysics が使う「数学角」（windX=cos, windZ=sin）。
 */
function bearingToAzimuthRad(fromDeg) {
  const b = ((fromDeg + 180) * Math.PI) / 180; // 吹いていく向き・北からの時計回り
  const worldX = Math.sin(b);
  const worldZ = -Math.cos(b);
  return Math.atan2(worldZ, worldX);
}

/** 実風速(m/s) → 斜めの傾き角。無風で約6°、約6m/sで既定の30°、13m/s超で60°上限。 */
function speedToTiltRad(ms) {
  const deg = Math.max(6, Math.min(60, 6 + ms * 4));
...
export class WeatherWind {
  ...
  _apply(ms, fromDeg, placeLabel) {
    ...
    this._wind.azimuthRad = bearingToAzimuthRad(fromDeg);
  ...
  _redraw() {
    this.panel.draw((c, w, h) => {
      ...
      c.fillText("本日の天気で雨の向きが変わります", w / 2, h * 0.32);

      c.font = "700 46px system-ui, sans-serif";
      if (this._wind.ok) {
        const s = this._wind.speedMs.toFixed(1);
        c.fillStyle = "#eaf1ff";
        c.fillText(`📍${this._wind.placeLabel}　${this._wind.dirLabel}の風 ${s} m/s`, w / 2, h * 0.64);
      } else {
        c.fillStyle = "#9fb4d6";
        c.font = "600 34px system-ui, sans-serif";
        c.fillText("天気を取得できませんでした（既定の風で進行）", w / 2, h * 0.64);
      }
    });
  }
